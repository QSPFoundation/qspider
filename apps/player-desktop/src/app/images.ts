...
export async function reagPng(path: string): Promise<ArrayBuffer> {
  const image = await Jimp.read(path);
  return image.bitmap.data;
}

export async function imageToIco(path: string): Promise<ArrayBuffer> {
  const image = await Jimp.read(path);
  const bitmap = image.bitmap;
  const size = bitmap.width;
  if (image._originalMime !== Jimp.MIME_PNG || size !== bitmap.height) {
    throw new Error('Only rectangle PNG images supported');
  }
  if (size !== 256) {
  ...
  const images = await Promise.all(resizedImages.concat(image));
  return imagesToIco(images);
}

function imagesToIco(images: Jimp[]): Buffer {
...
function getDir(img: Jimp, offset: number): Buffer {
...
function getBmpInfoHeader(img: Jimp): Buffer {
...
function getDib(img: Jimp): Buffer {
  const bitmap = img.bitmap;
